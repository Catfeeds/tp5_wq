...
public function task_member(){
        ...
        $data['task']=html_entity_decode(html_entity_decode(config('task')));

        if($task&&$params){
           $this->error('一次领一个', $url);
         }elseif($params){
        $member=new  Member;
         if(!$data){
        $this->error('暂时没有任务！');  } 
       ...
       if($count>=$this->user['ad_bag']){
        $this->error('广告包数目不足，无法领取更多任务！');
       }
...
public function task_member_submit(){
       ...
       // $validate = validate('Transfer');;
        ...
        M('history')->where("id=". $params['id'])->setfield('img_ids',$params['img_ids']);
         ...
         $this->assign('task',html_entity_decode(html_entity_decode(config('task'))));
         return $this->fetch();   
    }
...
public function task_member_img(){
       ...
       if($params[config]){
        $task_img[0]=config($params[config]);
         $task='';
