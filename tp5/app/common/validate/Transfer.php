...
////用     // onebase用
//  $validate = validate('Transfer');//使用助手函数实例化验证器使用助手函数实例化验证器  
//   $params['charge_member']='charge_member';//自己命名的规则  
//        if (!$validate->scene('charge_member')->check($params)) {// 应用场景//数据
...
//            return  $this->error($validate->getError());
 //       }
    ...
    protected $rule =   [
        ...
        //转账
        'to_user_id'=> 'require|transfer',
        'number'=> 'require|gt:0',

        //充值
        'charge_member'=> 'require|charge_member',
         
        //提现
        'withdrawadd'=>'require|withdrawadd',
   ...
   protected function transfer($value, $rule,  $params = []){
          $user=user();
...
protected function charge_member($value, $rule,  $params = []){
 ...
 protected function withdrawadd($value, $rule, $params = []){
	        $user=user();
	        $where['uid'] = $user['id'];
	        $where['status']=0;
 ...
 protected function re_uid($value, $rule, $data = []){
	...
	$arr3 = array();
	foreach ($arr1 as $k => $v) {
	    $arr3[] = array('num'=>$arr1[$k],'price'=>$arr2[$k]);
	}
	foreach ($arr3 as $key => $value) {
    $money=intval($value['num']['num'])*intval($value['price']['price']);
