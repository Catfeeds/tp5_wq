...
//var_dump($this->feeratio['agent_center'][0]);
...
public function  charge_member(){
       ...
       if(!$params['page']&&$params){
        ...
        $validate = validate('Transfer');
        $params['charge_member']='charge_member';
        if (!$validate->scene('charge_member')->check($params)) {
            return  $this->error($validate->getError());
        }
        ...
        $member->addencAdd($this->user,$params['number'],$bnum,$this->user);
        $this->success('充值申请成功！');
         }
        $this->assign('user',$this->user);
...
//var_dump($param);
       ...
       // var_dump($this->user['card']);
        //用     
       $validate = validate('Transfer');//助手函数调用这个类使用助手函数实例化验证器
        $param['withdrawadd']='withdrawadd';//自己命名的规则
       if (!$validate->scene('withdrawadd')->check($param)) {// 应用场景//数据
           return  $this->error($validate->getError());
       }
 ...
 /**
    ...
    //$man= M('member')->where('user_id=210991')->find();
 ...
 /**
    ...
    public function vip()
    {
      ...
      !session('member_info') && $this->redirect('admin/login/login');
       ...
       $this->assign('article_list', $this->logicArticle->getArticleList($where, 'a.*,m.nickname,c.name as category_name', 'a.create_time desc',3));
        $level=$this->user['agent'];
       $this->assign('u_level', $this->user['card_d'].$angent[$level]);
       $this->assign('today',$today);
      if($this->user['agent']<1&&$this->user['b4']>0){
       session('member_info.agent','1');
       }
    
      $variable= M('Member')->where('id>0')->select();
      foreach ($variable as $value) {
         if($value['agent']<1&&$value['b4']>0){
         M('member')->where('id='.$value['id'])->setfield('agent',1);}
      }
  ...
  /**
    ...
    public function recharge(){
        ...
        $username   = $this->request->param('username', '');
        $username2  = $this->request->param('username2', '');
        $money      = $this->request->param('money', 0, 'intval');

        IS_POST && $this->jump($this->logicFinance->recharge($type, $username2, $money));

     $this->assign('list', $this->logicFinance->getRechargeList($start, $end, $username));
     ...
     * 充值管理
     */
    public function checkRealname($username = ''){
        $this->jump($this->logicFinance->checkRealname($username));
    }
...
public function logout()
 ...
 //   }
    ...
    public function qcode(){
        ...
        $d="http://".$rootPath."/login/member_add/TPL/1/RID/".$user[id]."/FID/".$user[id];
...
//        $myName = $UserID."(".$username.") [".$HYJJ[$uLevel]."](".$user_tel.")";
        $myName = $UserID.'&nbsp;&nbsp;姓名：'.$username.'&nbsp;&nbsp;级别编号：'.$this->feeratio['is_agent'][$rs['is_agent']].'  推荐人：'.$rs['re_uid'].'  ';
        $myTabN = "m".$UID;
...
//          $z_myName = $rsuserid."(".$rusername.") [".$HYJJ[$rslv]."](".$user_tel2.")";
            $z_myName = $rsuserid.'&nbsp;&nbsp;姓名：'.$rusername.'&nbsp;&nbsp;级别编号：'.$this->feeratio['is_agent'][$rss['is_agent']].'   推荐人：'.$rss['re_uid'].'  '.$team[$tree];
...
//            $z_myName = $rsuserid."(".$rusername.") [".$HYJJ[$rslv]."]";
            // $z_myName = $rsuserid;
            $z_myName = $rsuserid.'&nbsp;&nbsp;姓名：'.$rusername.'&nbsp;&nbsp;级别编号：'.$this->feeratio['is_agent'][$rss['is_agent']].'   推荐人：'.$rss['re_uid'].'  '.$team[$rss['treeplace']];
            $z_tree[$zz][0] = '<img id="'.$oz_TabNN.'" src="'.$z_img.'" align="absmiddle" onclick="'.$z_function.'">';
            $z_tree[$zz][0].= '<img id="fg'.$rssid.'" src="'.$z_us_img.'" align="absmiddle"> ';
            $z_tree[$zz][0].= $z_myName;
            if(!empty($oz_img)){
