...
//var_dump($params);die;
  ...
  public function order_comfirm_get(){
       ...
       // if($this->user[b0]<$params['goods_cost']&&$params['pay_type']==0){ $this->error('余额不足请充值'); }
       ...
       switch ($ord['status']) {
                case 0:
                  ...
                  // $man=M('shop_order')->where('code="'.$params['code'].'"')->find();
                  // $data['id']=$man[uid];
                  // $menus->activate2($data); 


                  // $ord['status'] =$params['status']+1;
