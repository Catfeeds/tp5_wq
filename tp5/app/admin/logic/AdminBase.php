...
////?????
        foreach ($url_list as $key => $value) {
          $url_list[$key]= str_replace('_', '',$value);
        }
  
        if(!is_bool(strpos($s_url,'admin/vip'))||!is_bool(strpos($s_url,'admin/user'))){
             $s_url='admin/vip';//||strpos($s_url,'admin/user')
          } 
        if(!is_bool(strpos($s_url,'member/activate'))&&session('member_info')[is_agent]>=2){
             $s_url='admin/vip';
         // var_dump(session('member_info')[is_agent]);
        }
...
public function setDateWhere(&$where, $start, $end, $field){
    ...
    public function getIndexData()
    {
        ...
        // 产品信息
        $data['product_name']   = '高原圣果';
        $data['author']         = 'Bigotry';
        $data['website']        = 'www.onebase.org';
        $data['qun']            = '<a target="_blank" href="//shang.qq.com/wpa/qunwpa?idkey=3807aa892b97015a8e016778909dee8f23bbd54a4305d827482eda88fcc55b5e"><img border="0" src="//pub.idqqimg.com/wpa/images/group.png" alt="高原圣果 ①" title="高原圣果 ①"></a>';
