...
// 获取当前用户的ID
 ...
 /**
    ...
    public function memberAdd($data = [],$index=[])
    {
      ...
      $old_man=M('member')->where('father_uid="'.$data['father_uid'].'" and treeplace='.$data['TPL'])->find();
      $agent_man=M('member')->where('user_id="'.$data['agent_uid'].'"')->find();
      ...
      $result = $this->modelMember->setInfo($data);
        ...
        $add_data[] = ['member_id' => $new_man['id'], 'group_id' => 3];

        if($new_man['cpzj']>=50000){
         $add_data[] = ['member_id' => $new_man['id'], 'group_id' => 4];
        }
        ...
        //保存地址
         $data['uid']             = $new_man['id'];
         $data['default']         = 1;
        $getStatus=$this->modelAddress->setInfo($data);    

        //生成订单
         $data['num']             = 1;
       $data['code']='S'.date('YmdHis', time()).random_string(6, true, false, false, false);
         $data['create_time'] = time();
         $data['pay_type'] =0;
          $data['status'] =1;
         $data['name'] =$data['consignee_name'];
          $vo=M('shop')->where('price='.$data['cpzj'])->find();
          $data['goods_id']= $vo['id'];
          $data['goods_name']=$vo['name'];
          $data['goods_thumb']= $vo['thumb'];
          $data['goods_price']= $vo['price'];
    $data['address']= $data['province'].$data['city'].$data['county'].$data['address'];
         $resut=$this->modelShoporder->setInfo($data);
  
        
        return $result&& $getStatus ? [RESULT_SUCCESS, '会员添加成功', $url] : [RESULT_ERROR, $this->modelMember->getError()];


    }
 //统计单数
    public function jiandan($man){
       $ppath=$man['p_path'];
       $cpzj=$man['cpzj'];
...
$this->addencAdd($lrs,$cpzj,91,$man);
     ...
     public function activate2($params){

        $man= M('member')->where('id='.$params['id'])->find();
        $this->rw_bonus_dec(user(),21,$man['cpzj'],$man);
        $money_count=$this->feeratio['sj_ratio'][0]*$man['cpzj']/1000;
        $this->rw_bonus_inc($man,2,$money_count,$man);  
        $this->bonus($man,$money_count,TIME_NOW,'b2');
 
         $this->recoway($man);
         $this->juncway($man);
         $this->agent_center($man);
         $this->jiandan($man);
    
     //激活状态
      M('member')->where('id='.$params['id'])->setInc('is_pay');
 ...
 //接点方法，遍历出接点路径上的id
   ...
   public function recoway($man,$params){
       //1直推奖
        $reco_man=M('member')->where('id='.$man['re_id'])->find(); 
        $reco_money=$man['cpzj']*$this->feeratio['reco'][0]/100;
        $this->rw_bonus_inc($reco_man,1, $reco_money,$man);
        $this->bonus($reco_man,$reco_money, TIME_NOW, 'b1');
        //推荐路径
   ...
   //业绩奖      
    public function  achieve(){
      ...
      foreach ( $list as $achieve_man) {

         $p_achieve_money=min($achieve_man['l'],$achieve_man['r']);
            if($p_achieve_money>0){
           $achieve_money=($achieve_man['l']+$achieve_man['r'])*$this->feeratio['achievement'][0]/100;
           $achieve_money=min($achieve_money,$achieve_man['cpzj'],$achieve_man['l'],$achieve_man['r']);
            $this->rw_bonus_inc($achieve_man,3,$achieve_money,$achieve_man);
            $this->bonus($achieve_man,$achieve_money, TIME_NOW, 'b3');
            $this->manage($achieve_man,$achieve_money);
            }
           M('member')->where('id='.$achieve_man['id'])->setfield('l',0);
           M('member')->where('id='.$achieve_man['id'])->setfield('r',0);
        }
  ...
  //管理奖    
      public function  manage($man,$achieve_money){
   $lirs = M('member') ->where('id','in',$man['re_path'])->order('re_level desc')->select();
       foreach ($lirs as $lrs){
      $x_level=$man['re_level']-$lrs['re_level'];
      $money=$this->feeratio['manage'][$x_level]*$achieve_money/100;
       if($money>0){
      $this->rw_bonus_inc($lrs,4,$money,$man);
      $this->bonus($lrs,$money, TIME_NOW, 'b4');
         }
       }
     }

  //滑落奖    
      public function  see_you($lrs,$man){
       $x_level=$man['p_level']-$lrs['p_level'];
       $rx_level=$this->feeratio['see_you'][$lrs['cpzj']];
       if($rx_level>=$x_level){
        $money=$this->feeratio['see_you_ratio'][0]*$man['cpzj']/100;
        $this->rw_bonus_inc($lrs,5,$money,$man);
        $this->bonus($lrs,$money, TIME_NOW, 'b5');
         }
     }
...
//报单奖
    public function agent_center($man){  
      //报单奖
      $center=M('member')->where('user_id="'.$man['agent_uid'].'"')->find();
      $money=$man['cpzj']*$this->feeratio['agent_center'][0]/100;
      $this->rw_bonus_inc($center,6,$money,$man);
      $this->bonus($center,$money,TIME_NOW, 'b6');
    }
...
//先扣，然后发
 public function rw_bonus_inc($my,$bnum=0,$money_count=0,$man){
      ...
      $this->rw_bonus_dec($my,7,$money,$my);
      $this->bonus($my,-$money,TIME_NOW, 'b7');
      }
 //不扣手续费
    if($bnum==66){ $bnum=0; }
